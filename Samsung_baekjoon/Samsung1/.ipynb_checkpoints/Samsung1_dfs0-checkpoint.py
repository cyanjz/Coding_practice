...
def brute_force(b, cur_locs, count, min_count, board_hist):
    for i in range(4):
        ...
        if not goals[0] and not goals[1] and count < 10:
            ...
            if cur_locs != next_locs:
                board_hist_copy = copy.deepcopy(board_hist)
                board_hist_copy.append(b_copy)
                brute_force(b_copy, next_locs, count+1, min_count, board_hist_copy)
...
min_count = [11]
board_hist = [b]
brute_force(b, cur_locs, 1, min_count, board_hist)
if min_count[0] < 11:
    print(min_count[0])
else:
    print(-1)